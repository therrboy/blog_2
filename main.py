...
def load_user(user_id):
    return db.session.get(User, int(user_id))  # Se carga el usuario de la base de datos
...
class Comment(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"))  # Nueva columna para el ID del usuario
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            ...
            session[
                'logged_in'] = True  # Establece la variable de sesión para indicar que se ha iniciado sesión correctamente
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(
            email=email).first()  # Consulta en la base de datos si existe un usuario con este email.
...
def add_new_post():
    ...
    if not current_user.is_admin:
        abort(403)  # Devuelve un error HTTP 403 Forbidden si el usuario no es administrador
...
def edit_post(post_id):
    ...
    if edit_form.validate_on_submit():
        ...
        author = User.query.get(edit_form.author_id.data)
        post.author = author
